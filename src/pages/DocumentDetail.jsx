...
const DocumentDetail = () => {
  ...
  const { data: document, isLoading, error } = useGetDocumentBySlugQuery(slug)

  if (isLoading) return <Box p={4}>Đang tải document...</Box>
  if (error) return <Box p={4}>Có lỗi xảy ra khi tải document</Box>
  if (!document) return <Box p={4}>Không tìm thấy document</Box>
